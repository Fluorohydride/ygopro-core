...
int32 scriptlib::card_get_orb(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	lua_pushinteger(L, pcard->get_orb());
	return 1;
}
int32 scriptlib::card_get_sheer(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	lua_pushinteger(L, pcard->get_sheer());
	return 1;
}
...
int32 scriptlib::card_get_origin_level(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**) lua_touserdata(L, 1);
	if((pcard->data.type & (TYPE_XYZ | TYPE_LINK | TYPE_UNIVERSAL | TYPE_TIMESHEER)) || (pcard->status & STATUS_NO_LEVEL))
		lua_pushinteger(L, 0);
	else
...
int32 scriptlib::card_get_origin_orb(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	if (!(pcard->data.type & TYPE_UNIVERSAL))
		lua_pushinteger(L, 0);
	else
		lua_pushinteger(L, pcard->data.level);
	return 1;
}
int32 scriptlib::card_get_origin_sheer(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	if (!(pcard->data.type & TYPE_TIMESHEER))
		lua_pushinteger(L, 0);
	else
		lua_pushinteger(L, pcard->data.level);
	return 1;
...
int32 scriptlib::card_get_previous_orb_onfield(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	lua_pushinteger(L, pcard->previous.orb);
	return 1;
}
int32 scriptlib::card_get_previous_sheer_onfield(lua_State *L) {
	check_param_count(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	lua_pushinteger(L, pcard->previous.sheer);
	return 1;
}
...
int32 scriptlib::card_is_level(lua_State *L) {
	...
	card* pcard = *(card**)lua_touserdata(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	uint32 level = lua_tointeger(L, 2);
	if (pcard->get_level() == level)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
	return 1;
...
int32 scriptlib::card_is_rank(lua_State *L) {
	...
	card* pcard = *(card**)lua_touserdata(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	uint32 rank = lua_tointeger(L, 2);
	if (pcard->get_rank() == rank)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
	return 1;
...
int32 scriptlib::card_is_link(lua_State *L) {
	...
	card* pcard = *(card**)lua_touserdata(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	uint32 link = lua_tointeger(L, 2);
	if (pcard->get_link() == link)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
	return 1;
}
int32 scriptlib::card_is_orb(lua_State *L) {
	check_param_count(L, 2);
	card* pcard = *(card**)lua_touserdata(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	uint32 orb = lua_tointeger(L, 2);
	if (pcard->get_orb() == orb)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
	return 1;
}
int32 scriptlib::card_is_sheer(lua_State *L) {
	check_param_count(L, 2);
	card* pcard = *(card**)lua_touserdata(L, 1);
	check_param(L, PARAM_TYPE_CARD, 1);
	uint32 sheer = lua_tointeger(L, 2);
	if (pcard->get_sheer() == sheer)
		lua_pushboolean(L, 1);
	else
		lua_pushboolean(L, 0);
	return 1;
...
int32 scriptlib::card_is_level_below(lua_State *L) {
	...
	if(((pcard->data.type & TYPE_XYZ) && !(pcard->is_affected_by_effect(EFFECT_RANK_LEVEL) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL_S))) 
		|| (pcard->data.type & TYPE_LINK) || (pcard->data.type & TYPE_UNIVERSAL) || (pcard->data.type & TYPE_TIMESHEER) || (pcard->status & STATUS_NO_LEVEL)
		|| (!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !(pcard->current.location & LOCATION_ONFIELD)))
		lua_pushboolean(L, 0);
	else
...
int32 scriptlib::card_is_rank_below(lua_State *L) {
	...
	if(((!(pcard->data.type & TYPE_XYZ) && !(pcard->is_affected_by_effect(EFFECT_LEVEL_RANK) || pcard->is_affected_by_effect(EFFECT_LEVEL_RANK_S))) || (pcard->status & STATUS_NO_LEVEL)
	        || (!(pcard->data.type & TYPE_MONSTER) && !(pcard->current.location & LOCATION_ONFIELD))) || (pcard->data.type & TYPE_LINK) || (pcard->data.type & TYPE_UNIVERSAL) || (pcard->data.type & TYPE_TIMESHEER))
		lua_pushboolean(L, 0);
	else
...
int32 scriptlib::card_is_sheer_below(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	uint32 shr = lua_tointeger(L, 2);
	if (!(pcard->data.type & TYPE_TIMESHEER) || (pcard->status & STATUS_NO_LEVEL)
		|| (!(pcard->data.type & TYPE_MONSTER) && !(pcard->current.location & LOCATION_ONFIELD)))
		lua_pushboolean(L, 0);
	else
		lua_pushboolean(L, pcard->get_sheer() <= shr);
	return 1;
}
int32 scriptlib::card_is_sheer_above(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	uint32 shr = lua_tointeger(L, 2);
...
int32 scriptlib::card_is_orb_below(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	uint32 orb = lua_tointeger(L, 2);
	if (!(pcard->data.type & TYPE_UNIVERSAL) || (pcard->status & STATUS_NO_LEVEL)
		|| (!(pcard->data.type & TYPE_MONSTER) && !(pcard->current.location & LOCATION_ONFIELD)))
		lua_pushboolean(L, 0);
	else
		lua_pushboolean(L, pcard->get_orb() <= orb);
	return 1;
}
int32 scriptlib::card_is_orb_above(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	uint32 orb = lua_tointeger(L, 2);
...
int32 scriptlib::card_is_attack_below(lua_State *L) {
	...
	if(!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !(pcard->current.location & LOCATION_ONFIELD))
		lua_pushboolean(L, 0);
	else {
...
int32 scriptlib::card_is_can_be_universal_material(lua_State *L) {
	check_param_count(L, 3);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	card* scard = 0;
	...
	lua_pushboolean(L, pcard->is_can_be_universal_material(scard, playerid));
	return 1;
}
int32 scriptlib::card_is_can_be_sheer_material(lua_State *L) {
	check_param_count(L, 3);
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**)lua_touserdata(L, 1);
	card* scard = 0;
	...
	lua_pushboolean(L, pcard->is_can_be_sheer_material(scard, playerid));
	return 1;
}
