...
int32 scriptlib::card_is_set_card(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**) lua_touserdata(L, 1);
	uint32 set_code = (uint32)lua_tointeger(L, 2);
...
int32 scriptlib::card_is_dual_state(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**) lua_touserdata(L, 1);
	lua_pushboolean(L, (int)(ptr)pcard->is_affected_by_effect(EFFECT_DUAL_STATUS));
	return 1;
}
...
int32 scriptlib::card_set_flag_effect_label(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_CARD, 1);
	card* pcard = *(card**) lua_touserdata(L, 1);
	int32 code = (int32)(lua_tointeger(L, 2) & 0xfffffff) | 0x10000000;
...
int32 scriptlib::card_add_trap_monster_attribute(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_CARD, 1);
	int32 extra_type = (int32)lua_tointeger(L, 2);
