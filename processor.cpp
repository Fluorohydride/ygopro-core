		...
		if(it->step == 0) {
			...
			if(core.continuous_chain.size()) {
				...
				for(auto& pcard : pret->container)
					pcard->create_relation(core.current_chain.back());
...
int32 field::process_idle_command(uint16 step) {
	...
	case 0: {
		...
		for(int i = 0; i < 5; ++i) {
			auto pcard = player[infos.turn_player].list_mzone[i];
...
int32 field::process_battle_command(uint16 step) {
	...
	case 10: {
		...
		uint32 acon = (uint32)core.units.begin()->arg2 >> 16;
		uint32 afid = (uint32)core.units.begin()->arg2 & 0xffff;
...
int32 field::sort_chain(uint16 step, uint8 tp) {
	...
	case 1: {
		...
		if(tp) {
			...
			count = (int32)core.ntpchain.size();
...
int32 field::solve_chain(uint16 step, uint32 chainend_arg1, uint32 chainend_arg2) {
	...
	case 3: {
		...
		if(core.units.begin()->peffect) {
			peffect->operation = (int32)(ptr)core.units.begin()->peffect;
		}
...
int32 field::refresh_location_info(uint16 step) {
	...
	case 4: {
		uint32 count1 = (uint32)core.units.begin()->arg1;
		uint32 selflag = 0;
		uint8 s, pt = 0;
