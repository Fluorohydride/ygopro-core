...
int32 scriptlib::_duel_check_release_group(lua_State *L, int32 use_hand) {
	...
	duel* pduel = interpreter::get_duel_info(L);
	int32 result = pduel->game_field->check_release_list(playerid, fcount, use_con, use_hand, 3, extraargs, pexception, pexgroup, reason);
	pduel->game_field->core.must_select_cards.clear();
	lua_pushboolean(L, result);
	return 1;
}
int32 scriptlib::duel_check_release_group(lua_State *L) {
	return _duel_check_release_group(L, FALSE);
}
int32 scriptlib::duel_check_release_group_ex(lua_State *L) {
	return _duel_check_release_group(L, TRUE);
}
int32 scriptlib::_duel_select_release_group(lua_State *L, int32 use_hand) {
	...
	pduel->game_field->get_release_list(playerid, &pduel->game_field->core.release_cards, &pduel->game_field->core.release_cards_ex, &pduel->game_field->core.release_cards_ex_oneof, use_con, use_hand, 3, extraargs, pexception, pexgroup, reason);
...
int32 scriptlib::duel_select_release_group(lua_State *L) {
	return _duel_select_release_group(L, FALSE);
}
int32 scriptlib::duel_select_release_group_ex(lua_State *L) {
	return _duel_select_release_group(L, TRUE);
}
/**
...
int32 scriptlib::_duel_get_ritual_material(lua_State *L, uint8 no_level) {
	check_param_count(L, 1);
	int32 playerid = (int32)lua_tointeger(L, 1);
	if(playerid != 0 && playerid != 1)
		return 0;
	duel* pduel = interpreter::get_duel_info(L);
	group* pgroup = pduel->new_group();
	pduel->game_field->get_ritual_material(playerid, pduel->game_field->core.reason_effect, &pgroup->container, no_level);
	interpreter::group2value(L, pgroup);
	return 1;
}
int32 scriptlib::duel_get_ritual_material(lua_State *L) {
	return _duel_get_ritual_material(L, FALSE);
}
int32 scriptlib::duel_get_ritual_material_ex(lua_State *L) {
	return _duel_get_ritual_material(L, TRUE);
}
