...
public:
	...
	void set_control(card* pcard, uint8 playerid, uint16_t reset_phase, uint8 reset_count);

	int32_t get_pzone_sequence(uint8 pseq) const;
	card* get_field_card(uint8 playerid, uint32_t general_location, uint8 sequence) const;
	int32_t is_location_useable(uint8 playerid, uint32_t general_location, uint8 sequence) const;
	int32_t get_useable_count(card* pcard, uint8 playerid, uint8 location, uint8 uplayer, uint32_t reason, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_useable_count_fromex(card* pcard, uint8 playerid, uint8 uplayer, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_spsummonable_count(card* pcard, uint8 playerid, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_spsummonable_count_fromex(card* pcard, uint8 playerid, uint8 uplayer, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_useable_count_other(card* pcard, uint8 playerid, uint8 location, uint8 uplayer, uint32_t reason, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_tofield_count(card* pcard, uint8 playerid, uint8 location, uint32_t uplayer, uint32_t reason, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_useable_count_fromex_rule4(card* pcard, uint8 playerid, uint8 uplayer, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_spsummonable_count_fromex_rule4(card* pcard, uint8 playerid, uint8 uplayer, uint32_t zone = 0xff, uint32_t* list = nullptr);
	int32_t get_mzone_limit(uint8 playerid, uint8 uplayer, uint32_t reason);
	int32_t get_szone_limit(uint8 playerid, uint8 uplayer, uint32_t reason);
	uint32_t get_linked_zone(int32_t playerid);
	uint32_t get_rule_zone_fromex(int32_t playerid, card* pcard);
	void filter_must_use_mzone(uint8 playerid, uint8 uplayer, uint32_t reason, card* pcard, uint32_t* flag);
	...
	void filter_field_effect(uint32_t code, effect_set* eset, uint8 sort = TRUE);
	void filter_affected_cards(effect* peffect, card_set* cset);
	void filter_inrange_cards(effect* peffect, card_set* cset);
	void filter_player_effect(uint8 playerid, uint32_t code, effect_set* eset, uint8 sort = TRUE);
	int32_t filter_matching_card(lua_State* L, int32_t findex, uint8 self, uint32_t location1, uint32_t location2, group* pgroup, card* pexception, group* pexgroup, uint32_t extraargs, card** pret = nullptr, int32_t fcount = 0, int32_t is_target = FALSE);
	int32_t filter_field_card(uint8 self, uint32_t location, uint32_t location2, group* pgroup);
	effect* is_player_affected_by_effect(uint8 playerid, uint32_t code);

	int32_t get_release_list(lua_State* L, uint8 playerid, card_set* release_list, card_set* ex_list, card_set* ex_list_oneof, int32_t use_con, int32_t use_hand, int32_t fun, int32_t exarg, card* exc, group* exg, uint32_t reason);
	int32_t check_release_list(lua_State* L, uint8 playerid, int32_t count, int32_t use_con, int32_t use_hand, int32_t fun, int32_t exarg, card* exc, group* exg, uint32_t reason);
	...
	void get_fusion_material(uint8 playerid, card_set* material_all, card_set* material_base, uint32_t location);
	...
	int32_t check_lp_cost(uint8 playerid, uint32_t cost, uint32_t must_pay);
	void save_lp_cost() {}
	void restore_lp_cost() {}
	int32_t pay_lp_cost(uint32_t step, uint8 playerid, uint32_t cost, uint32_t must_pay);

	uint32_t get_field_counter(uint8 self, uint8 s, uint8 o, uint16_t countertype);
	...
	int32_t get_must_material_list(uint8 playerid, uint32_t limit, card_set* must_list);
	int32_t check_must_material(group* mg, uint8 playerid, uint32_t limit);
	...
	int32_t check_tribute(card* pcard, int32_t min, int32_t max, group* mg, uint8 toplayer, uint32_t zone = 0x1f, uint32_t releasable = 0xff00ff, uint32_t pos = 0x1);
	...
	int32_t is_player_can_discard_hand(uint8 playerid, card* pcard, effect* reason_effect, uint32_t reason);
	int32_t is_player_can_action(uint8 playerid, uint32_t actionlimit);
	int32_t is_player_can_summon(uint32_t sumtype, uint8 playerid, card* pcard, uint8 toplayer);
	int32_t is_player_can_mset(uint32_t sumtype, uint8 playerid, card* pcard, uint8 toplayer);
	int32_t is_player_can_sset(uint8 playerid, card* pcard);
	int32_t is_player_can_spsummon(uint8 playerid);
	int32_t is_player_can_spsummon(effect* reason_effect, uint32_t sumtype, uint8 sumpos, uint8 playerid, uint8 toplayer, card* pcard);
	int32_t is_player_can_flipsummon(uint8 playerid, card* pcard);
	int32_t is_player_can_spsummon_monster(uint8 playerid, uint8 toplayer, uint8 sumpos, uint32_t sumtype, card_data* pdata);
	int32_t is_player_can_spsummon_count(uint8 playerid, uint32_t count);
	int32_t is_player_can_release(uint8 playerid, card* pcard, uint32_t reason);
	int32_t is_player_can_place_counter(uint8 playerid, card* pcard, uint16_t countertype, uint16_t count);
	int32_t is_player_can_remove_counter(uint8 playerid, card* pcard, uint8 s, uint8 o, uint16_t countertype, uint16_t count, uint32_t reason);
	int32_t is_player_can_remove_overlay_card(uint8 playerid, card* pcard, uint8 s, uint8 o, uint16_t count, uint32_t reason);
	...
	int32_t is_player_can_remove(uint8 playerid, card* pcard, uint32_t reason);
	...
	uint32_t process();
	int32_t execute_cost(uint16_t step, effect* peffect, uint8 triggering_player);
	int32_t execute_operation(uint16_t step, effect* peffect, uint8 triggering_player);
	int32_t execute_target(uint16_t step, effect* peffect, uint8 triggering_player);
	void raise_event(card* event_card, uint32_t event_code, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 event_player, uint32_t event_value);
	void raise_event(const card_set& event_cards, uint32_t event_code, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 event_player, uint32_t event_value);
	void raise_single_event(card* trigger_card, card_set* event_cards, uint32_t event_code, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 event_player, uint32_t event_value);
	...
	int32_t process_turn(uint16_t step, uint8 turn_player);
	...
	void send_to(const card_set& targets, effect* reason_effect, uint32_t reason, uint32_t reason_player, uint32_t playerid, uint32_t destination, uint32_t sequence, uint32_t position, uint8 send_activating = FALSE);
	void send_to(card* target, effect* reason_effect, uint32_t reason, uint32_t reason_player, uint32_t playerid, uint32_t destination, uint32_t sequence, uint32_t position, uint8 send_activating = FALSE);
	...
	void select_tribute_cards(card* target, uint8 playerid, uint8 cancelable, int32_t min, int32_t max, uint8 toplayer, uint32_t zone);

	// summon
	int32_t summon(uint16_t step, uint8 sumplayer, card* target, effect* proc, uint8 ignore_count, uint8 min_tribute, uint32_t zone, uint32_t action_type);
	int32_t mset(uint16_t step, uint8 setplayer, card* ptarget, effect* proc, uint8 ignore_count, uint8 min_tribute, uint32_t zone, uint32_t action_type);
	int32_t flip_summon(uint16_t step, uint8 sumplayer, card* target, uint32_t action_type);
	int32_t special_summon_rule(uint16_t step, uint8 sumplayer, card* target, uint32_t summon_type, uint32_t action_type);

	int32_t remove_counter(uint16_t step, uint32_t reason, card* pcard, uint8 rplayer, uint8 s, uint8 o, uint16_t countertype, uint16_t count);
	int32_t remove_overlay_card(uint16_t step, uint32_t reason, card* pcard, uint8 rplayer, uint8 s, uint8 o, uint16_t min, uint16_t max);
	int32_t get_control(uint16_t step, effect* reason_effect, uint8 reason_player, group* targets, uint8 playerid, uint16_t reset_phase, uint8 reset_count, uint32_t zone);
	int32_t swap_control(uint16_t step, effect* reason_effect, uint8 reason_player, group* targets1, group* targets2, uint16_t reset_phase, uint8 reset_count);
	int32_t self_destroy(uint16_t step, card* ucard, int32_t p);
	int32_t trap_monster_adjust(uint16_t step);
	int32_t equip(uint16_t step, uint8 equip_player, card* equip_card, card* target, uint32_t up, uint32_t is_step);
	int32_t draw(uint16_t step, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 playerid, int32_t count);
	int32_t damage(uint16_t step, effect* reason_effect, uint32_t reason, uint8 reason_player, card* reason_card, uint8 playerid, int32_t amount, uint32_t is_step);
	int32_t recover(uint16_t step, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 playerid, int32_t amount, uint32_t is_step);
	int32_t sset(uint16_t step, uint8 setplayer, uint8 toplayer, card* ptarget, effect* reason_effect);
	int32_t sset_g(uint16_t step, uint8 setplayer, uint8 toplayer, group* ptarget, uint8 confirm, effect* reason_effect);
	int32_t special_summon_step(uint16_t step, group* targets, card* target, uint32_t zone);
	int32_t special_summon(uint16_t step, effect* reason_effect, uint8 reason_player, group* targets, uint32_t zone);
	int32_t destroy_replace(uint16_t step, group* targets, card* target, uint8 battle);
	int32_t destroy(uint16_t step, group* targets, effect* reason_effect, uint32_t reason, uint8 reason_player);
	int32_t release_replace(uint16_t step, group* targets, card* target);
	int32_t release(uint16_t step, group* targets, effect* reason_effect, uint32_t reason, uint8 reason_player);
	int32_t send_replace(uint16_t step, group* targets, card* target);
	int32_t send_to(uint16_t step, group* targets, effect* reason_effect, uint32_t reason, uint8 reason_player, uint8 send_activating);
	int32_t discard_deck(uint16_t step, uint8 playerid, uint8 count, uint32_t reason);
	int32_t move_to_field(uint16_t step, card* target, uint32_t enable, uint32_t ret, uint32_t pzone, uint32_t zone);
	int32_t change_position(uint16_t step, group* targets, effect* reason_effect, uint8 reason_player, uint32_t enable);
	...
	int32_t select_xyz_material(int16_t step, uint8 playerid, uint32_t lv, card* pcard, int32_t min, int32_t max);
	int32_t select_release_cards(int16_t step, uint8 playerid, uint8 cancelable, int32_t min, int32_t max);
	int32_t select_tribute_cards(int16_t step, card* target, uint8 playerid, uint8 cancelable, int32_t min, int32_t max, uint8 toplayer, uint32_t zone);
	int32_t toss_coin(uint16_t step, effect* reason_effect, uint8 reason_player, uint8 playerid, int32_t count);
	int32_t toss_dice(uint16_t step, effect* reason_effect, uint8 reason_player, uint8 playerid, uint8 count1, uint8 count2);
	int32_t rock_paper_scissors(uint16_t step, uint8 repeat);

	bool check_response(size_t vector_size, int32_t min_len, int32_t max_len) const;
	int32_t select_battle_command(uint16_t step, uint8 playerid);
	int32_t select_idle_command(uint16_t step, uint8 playerid);
	int32_t select_effect_yes_no(uint16_t step, uint8 playerid, uint32_t description, card* pcard);
	int32_t select_yes_no(uint16_t step, uint8 playerid, uint32_t description);
	int32_t select_option(uint16_t step, uint8 playerid);
	int32_t select_card(uint16_t step, uint8 playerid, uint8 cancelable, uint8 min, uint8 max);
	int32_t select_unselect_card(uint16_t step, uint8 playerid, uint8 cancelable, uint8 min, uint8 max, uint8 finishable);
	int32_t select_chain(uint16_t step, uint8 playerid, uint8 spe_count, uint8 forced);
	int32_t select_place(uint16_t step, uint8 playerid, uint32_t flag, uint8 count);
	int32_t select_position(uint16_t step, uint8 playerid, uint32_t code, uint8 positions);
	int32_t select_tribute(uint16_t step, uint8 playerid, uint8 cancelable, uint8 min, uint8 max);
	int32_t select_counter(uint16_t step, uint8 playerid, uint16_t countertype, uint16_t count, uint8 s, uint8 o);
