...
uint32 card::get_infos(byte* buf, int32 query_flag, int32 use_cache) {
	...
	if(!use_cache) {
		...
		if(query_flag & QUERY_ORB) q_cache.orb = *p++ = get_orb();
		if(query_flag & QUERY_SHEER) q_cache.sheer = *p++ = get_sheer();
		...
		if((query_flag & QUERY_ORB) && ((uint32)(tdata = get_orb()) != q_cache.orb)) {
			q_cache.orb = tdata;
			*p++ = tdata;
		}
		else query_flag &= ~QUERY_ORB;
		if ((query_flag & QUERY_SHEER) && ((uint32)(tdata = get_sheer()) != q_cache.sheer)) {
			q_cache.sheer = tdata;
			*p++ = tdata;
		}
		else query_flag &= ~QUERY_SHEER;
...
int32 card::get_base_attack() {
	if(!(data.type & TYPE_MONSTER) && !(get_type() & TYPE_MONSTER) && !is_affected_by_effect(EFFECT_PRE_MONSTER))
		return 0;
	if (current.location != LOCATION_ONFIELD || get_status(STATUS_SUMMONING | STATUS_SPSUMMON_STEP))
		return data.attack;
...
uint32 card::get_orb() {
	if (!(data.type & TYPE_UNIVERSAL) || (status & STATUS_NO_LEVEL))
		return 0;
	if (assume_type == ASSUME_ORB)
		return assume_value;
	if (!(current.location & LOCATION_ONFIELD))
		return data.level;
	if (temp.level != 0xffffffff)
		return temp.level;
	effect_set effects;
	int32 orb = data.level;
	temp.level = orb;
	int32 up = 0, upc = 0;
	filter_effect(EFFECT_UPDATE_ORB, &effects, FALSE);
	filter_effect(EFFECT_CHANGE_ORB, &effects, FALSE);
	filter_effect(EFFECT_CHANGE_ORB_FINAL, &effects);
	for (int32 i = 0; i < effects.size(); ++i) {
		switch (effects[i]->code) {
		case EFFECT_UPDATE_ORB:
			if ((effects[i]->type & EFFECT_TYPE_SINGLE) && !effects[i]->is_flag(EFFECT_FLAG_SINGLE_RANGE))
				up += effects[i]->get_value(this);
			else
				upc += effects[i]->get_value(this);
			break;
		case EFFECT_CHANGE_ORB:
			orb = effects[i]->get_value(this);
			up = 0;
			break;
		case EFFECT_CHANGE_ORB_FINAL:
			orb = effects[i]->get_value(this);
			up = 0;
			upc = 0;
			break;
		}
		temp.level = orb + up + upc;
	}
	orb += up + upc;
	if (orb < 1 && (get_type() & TYPE_MONSTER) && !is_affected_by_effect(EFFECT_ALLOW_NEGATIVE))
		orb = 1;
	temp.level = 0xffffffff;
	return orb;
}
uint32 card::get_sheer() {
	if (!(data.type & TYPE_TIMESHEER) || (status & STATUS_NO_LEVEL))
		return 0;
	if (assume_type == ASSUME_SHEER)
		return assume_value;
	if (!(current.location & LOCATION_ONFIELD))
		return data.level;
	if (temp.level != 0xffffffff)
		return temp.level;
	effect_set effects;
	int32 sheer = data.level;
	temp.level = sheer;
	int32 up = 0, upc = 0;
	filter_effect(EFFECT_UPDATE_SHEER, &effects, FALSE);
	filter_effect(EFFECT_CHANGE_SHEER, &effects, FALSE);
	filter_effect(EFFECT_CHANGE_SHEER_FINAL, &effects);
	for (int32 i = 0; i < effects.size(); ++i) {
		switch (effects[i]->code) {
		case EFFECT_UPDATE_SHEER:
			if ((effects[i]->type & EFFECT_TYPE_SINGLE) && !effects[i]->is_flag(EFFECT_FLAG_SINGLE_RANGE))
				up += effects[i]->get_value(this);
			else
				upc += effects[i]->get_value(this);
			break;
		case EFFECT_CHANGE_SHEER:
			sheer = effects[i]->get_value(this);
			up = 0;
			break;
		case EFFECT_CHANGE_SHEER_FINAL:
			sheer = effects[i]->get_value(this);
			up = 0;
			upc = 0;
			break;
		}
		temp.level = sheer + up + upc;
	}
	sheer += up + upc;
	if (sheer < 1 && (get_type() & TYPE_MONSTER) && !is_affected_by_effect(EFFECT_ALLOW_NEGATIVE))
		sheer = 1;
	temp.level = 0xffffffff;
	return sheer;
}
uint32 card::get_synchro_level(card* pcard) {
	...
	|| (data.type & TYPE_LINK) || (data.type & TYPE_UNIVERSAL) || (data.type & TYPE_TIMESHEER))
		return 0;
	uint32 lev;
	effect_set eset;
	...
	for (int32 i = 0; i < effects.size(); ++i) {
		...
		if (effects[i]->operation) {
			...
			if (effects[i]->code == EFFECT_ADD_ATTRIBUTE)
				altattribute |= effects[i]->get_value(this);
			else
				altattribute &= ~(effects[i]->get_value(this));
		} else {
			if (effects[i]->code == EFFECT_ADD_ATTRIBUTE)
	...
	for (int32 i = 0; i < effects.size(); ++i) {
		...
		if (effects[i]->operation) {
			...
			if (effects[i]->code == EFFECT_ADD_RACE)
				altrace |= effects[i]->get_value(this);
			else
				altrace &= ~(effects[i]->get_value(this));
		} else {
			if (effects[i]->code == EFFECT_ADD_RACE)
...
int32 card::is_capable_send_to_hand(uint8 playerid) {
	if(is_status(STATUS_LEAVE_CONFIRMED))
		return FALSE;
	if((current.location == LOCATION_EXTRA) && (data.type & (TYPE_FUSION + TYPE_SYNCHRO + TYPE_XYZ + TYPE_LINK + TYPE_UNIVERSAL + TYPE_TIMESHEER)))
		return FALSE;
...
int32 card::is_capable_send_to_extra(uint8 playerid) {
	if(!(data.type & (TYPE_FUSION | TYPE_SYNCHRO | TYPE_XYZ | TYPE_PENDULUM | TYPE_LINK | TYPE_UNIVERSAL | TYPE_TIMESHEER)))
		return FALSE;
	if(is_affected_by_effect(EFFECT_CANNOT_TO_DECK))
		return FALSE;
	if(!pduel->game_field->is_player_can_send_to_deck(playerid, this))
...
int32 card::is_capable_cost_to_grave(uint8 playerid) {
	...
	if(data.type & TYPE_TOKEN)
		return FALSE;
	if((data.type & TYPE_PENDULUM | TYPE_TIMESHEER) && (current.location & LOCATION_ONFIELD) && !is_affected_by_effect(EFFECT_CANNOT_TO_DECK))
		return FALSE;
...
int32 card::is_capable_cost_to_hand(uint8 playerid) {
	...
	if(data.type & (TYPE_TOKEN | TYPE_FUSION | TYPE_SYNCHRO | TYPE_XYZ | TYPE_LINK | TYPE_UNIVERSAL | TYPE_TIMESHEER))
		return FALSE;
	if(current.location == LOCATION_HAND)
		return FALSE;
	if(is_affected_by_effect(EFFECT_CANNOT_USE_AS_COST))
...
int32 card::is_capable_cost_to_extra(uint8 playerid) {
	uint32 redirect = 0;
	uint32 dest = LOCATION_DECK;
	if(!(data.type & (TYPE_FUSION | TYPE_SYNCHRO | TYPE_XYZ | TYPE_LINK | TYPE_UNIVERSAL | TYPE_TIMESHEER)))
		return FALSE;
...
int32 card::is_capable_attack() {
	if(!is_position(POS_FACEUP_ATTACK) && !(is_position(POS_FACEUP_DEFENSE) && is_affected_by_effect(EFFECT_DEFENSE_ATTACK)))
		return FALSE;
	if(!(current.location == LOCATION_SZONE) && is_affected_by_effect(EFFECT_SZONE_ATTACK))
		return FALSE;
	if(is_affected_by_effect(EFFECT_FORBIDDEN))
		return FALSE;
...
int32 card::is_can_be_synchro_material(card* scard, uint8 playerid, card* tuner) {
	if(data.type & (TYPE_XYZ | TYPE_LINK | TYPE_UNIVERSAL | TYPE_TIMESHEER) && !(is_affected_by_effect(EFFECT_RANK_LEVEL) || is_affected_by_effect(EFFECT_RANK_LEVEL_S)))
		return FALSE;
...
int32 card::is_can_be_universal_material(card* scard, uint8 playerid) {
	if(!(get_type(scard, SUMMON_TYPE_UNIVERSAL, playerid) & TYPE_MONSTER))
		return FALSE;
	if(is_status(STATUS_FORBIDDEN))
		return FALSE;
	effect_set eset;
	filter_effect(EFFECT_CANNOT_BE_UNIVERSAL_MATERIAL, &eset);
	for(int32 i = 0; i < eset.size(); ++i)
		if(eset[i]->get_value(scard))
			return FALSE;
	return TRUE;
}
int32 card::is_can_be_sheer_material(card* scard, uint8 playerid) {
	if(!(data.attribute & ATTRIBUTE_TIME))
		return FALSE;
	if(!(get_type(scard, SUMMON_TYPE_TIMESHEER, playerid) & TYPE_MONSTER))
		return FALSE;
	if(is_status(STATUS_FORBIDDEN))
		return FALSE;
	effect_set eset;
	filter_effect(EFFECT_CANNOT_BE_SHEER_MATERIAL, &eset);
	for (int32 i = 0; i < eset.size(); ++i)
		if (eset[i]->get_value(scard))
			return FALSE;
	return TRUE;
